...
class Avatar:
    ...
    def __init__(self, args, avatar_id, video_path, bbox_shift, batch_size, preparation, vae, fp):
        try:
...
def prepare_material(self):
    """
    Prepare all the materials required for avatar animation, optimized with ThreadPoolExecutor.

    Steps:
        1. Save avatar metadata.
        2. Extract or copy images.
        3. Extract landmarks and bounding boxes.
        4. Apply extra margin and process frames in parallel.
        5. Generate latent representations using VAE.
        6. Create and save masks, coordinates, and latents.

    This version parallelizes frame processing to significantly reduce runtime.
    """
    try:
        logging.info(f"[INFO] Preparing materials for avatar: {self.avatar_id}")
        ...
        # --- Step 4: Define frame processing function ---
        def process_frame(idx, bbox, frame):
            """Process a single frame: crop, resize, encode to latent."""
            try:
                if bbox == coord_placeholder:
                    return None
                x1, y1, x2, y2 = bbox
                ...
                logging.error(f"[ERROR] Frame {idx} failed: {e}")
                return None

        # --- Step 5: Parallel processing of frames ---
        logging.info("[INFO] Processing frames in parallel...")
        with ThreadPoolExecutor(max_workers=os.cpu_count() or 4) as executor:
            futures = [executor.submit(process_frame, i, bbox, frame)
                       for i, (bbox, frame) in enumerate(zip(coord_list, frame_list))]

            for f in tqdm(as_completed(futures), total=len(futures), desc="Encoding frames"):
                result = f.result()
                if result is not None:
                    input_latent_list.append(result)

        # --- Step 6: Create cycles for animation ---
        self.frame_list_cycle = frame_list + frame_list[::-1]
        self.coord_list_cycle = coord_list + coord_list[::-1]
        self.input_latent_list_cycle = input_latent_list + input_latent_list[::-1]
        self.mask_coords_list_cycle = []
        ...
        logging.info(f"[OK] Processed {len(input_latent_list)} frames successfully.")

        # --- Step 7: Generate and save masks ---
        logging.info("[INFO] Generating facial masks...")
        for i, frame in enumerate(tqdm(self.frame_list_cycle, desc="Generating masks")):
            cv2.imwrite(f"{self.full_imgs_path}/{str(i).zfill(8)}.png", frame)

            x1, y1, x2, y2 = self.coord_list_cycle[i]
            mode = getattr(self.args, "parsing_mode", "raw") if getattr(self.args, "version", "v1") == "v15" else "raw"
